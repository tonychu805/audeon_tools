...
def process_ssml_files(input_dir, output_dir, provider="google", voice=None, audio_format="MP3"):
    """
    Process all SSML files in input directory
    """
    # Find all SSML files
    ...
    if not ssml_files:
        print(f"No SSML files found in {input_dir}")
        return
    ...
    for ssml_file in ssml_files:
        ...
        rel_path = os.path.relpath(ssml_file, input_dir)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"✗ Error processing {ssml_file}")
                print(f"  Error: {result.stderr}")
                error_count += 1
                
        except Exception as e:
            print(f"✗ Exception processing {ssml_file}: {e}")
            error_count += 1
    
    print(f"\nBatch processing complete:")
    print(f"  Success: {success_count}")
    print(f"  Errors: {error_count}")
    print(f"  Total: {len(ssml_files)}")

def main():
    parser = argparse.ArgumentParser(description='Batch TTS processor for SSML files')
    
    parser.add_argument('input_dir', help='Input directory containing SSML files')
    parser.add_argument('output_dir', help='Output directory for audio files')
    parser.add_argument('--provider', choices=['google', 'elevenlabs', 'minimax'], 
